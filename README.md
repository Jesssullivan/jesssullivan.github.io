...
- Shared-cache actions execute locally on the runner with stable writable `/tmp` HOME/XDG paths. This keeps Tectonic and other cache-using tools off the ARC image's read-only `/root` without introducing runner-specific paths into Bazel action keys.
