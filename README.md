...
- Shared-cache CI bounds local Bazel parallelism to four jobs and serializes local test actions. This protects the 4 GiB ARC pod from cold-graph and concurrent-browser memory pressure without changing the independently managed capacity of explicit executor-backed proofs.
