...
\begin{itemize}
    ...
    \item \textbf{Web \& Inference:} \tech{Flask}, \tech{TypeScript} (\cvlink{http://fft.js}{fft.js}, \tech{Leaflet}, \tech{React}, \tech{Vue}, \tech{Docker}, \tech{WebAssembly}), live demos in \tech{React Native} and \tech{Swift}
    \item \textbf{Infrastructure:} Confluence + BitBucket, EC2 \& Heroku, production model deployment at scale
\end{itemize}
...
Developed and taught rapid fabrication curricula for DLA students and faculty. \tech{OpenSCAD}, \tech{Fusion 360}, \tech{Inkscape}, \tech{C++}. GitHub/Linear project management.
...
Long term committer, member and supporter of numerous organizations including \textbf{Futhark} (WebGPU/WGSL compiler backend---modernized C runtime for Emscripten 4.x/Dawn, authored WebGPU test suite), \textbf{Rocky Enterprise Linux Foundation} (Community Team and kernel SIG, AltArch SIG), \textbf{Liqo} (kubernetes topology fabric, used by \textbf{CERN}), The \textbf{Apache Foundation} (ASF \textbf{Solr} web security), \textbf{Caddy} (and xcaddy build tooling), \textbf{libdns} (ACME \& DNS integrations), \textbf{SearXNG}, \tech{Skeleton UI}, \textbf{Klipper}, \textbf{Joplin}, \textbf{FFT.js}, \textbf{KeePassXC}, \tech{svelte-superforms} and the creation of numerous FOSS automation tools and GIS utilities.
...
\begin{itemize}
    ...
    \item \textbf{HPC:} \tech{Chapel}, \tech{Haskell}, \tech{Futhark} (WGSL/WebGPU compiler backend). Performance-oriented systems and property-based testing.
\end{itemize}
...
I have begun learning the \tech{Nix} build system and have started to dip my toes in \tech{Rust} SIMD development. Active contributor to \textbf{Futhark}'s WebGPU compiler backend---modernizing the WGSL code generation pipeline, writing the WebGPU test suite, and working toward mainlining the backend. Deeper WASM integration across the stack.
...
\begin{itemize}
    ...
    \item \textbf{Outbot} --- Repo management SLM. Zoned summaries, conflict resolution, upstream PR analysis. Python with composable factory architecture. Slated for debut on GitLab Duo Agent Platform.
