...
\begin{itemize}
    \item \textbf{Reverse Engineering \& Binary Analysis:} \tech{Ghidra}, \tech{Frida}, \tech{ILSpy}, \tech{Mitre Caldera}, \tech{Zig} --- firmware RE and NVMe controller recovery via USB bridge XRAM injection (see \S\ref{sec:publications}). 5,400 LOC open-source Zig tool for bypassing ASMedia ASM2362 opcode whitelists to recover write-protected NVMe SSDs over USB.
    \item \textbf{Zig capability libraries} with C ABI surfaces and cross-platform builds: \tech{zig-crypto} (hashing, HMAC, AES-CBC, PBKDF2, P-256 ECDH, Ed25519, CSPRNG), \tech{zig-notify} (desktop notifications), \tech{zig-keychain} (generic secret storage), \tech{zig-ctap2} (CTAP2 over USB HID). A boring native boundary that audits, cross-compiles, and calls from anything that speaks C.
...
\cvlink{https://github.com/Jesssullivan/pixelwise-research}{pixelwise-research} --- ESDT Monads and pixelwise classification research.
