...
export class GitHubService extends ServiceMap.Service<GitHubService, {
	...
	readonly postReview: (owner: string, repo: string, prNumber: number, commitId: string, comments: ReviewComment[]) => Effect.Effect<void, Error>
	readonly postComment: (owner: string, repo: string, issueNumber: number, body: string, marker?: string) => Effect.Effect<void, Error>
	readonly addLabels: (owner: string, repo: string, issueNumber: number, labels: string[]) => Effect.Effect<void, Error>
	readonly removeLabel: (owner: string, repo: string, issueNumber: number, label: string) => Effect.Effect<void, Error>
	readonly reactToComment: (owner: string, repo: string, commentId: number, reaction: "+1" | "-1" | "eyes") => Effect.Effect<void, Error>
	readonly isCollaborator: (owner: string, repo: string, username: string) => Effect.Effect<boolean, Error>
	readonly commitFileChange: (owner: string, repo: string, path: string, content: string, sha: string, branch: string, message: string) => Effect.Effect<void, Error>
}>()("GitHubService") {}
...
export const GitHubServiceLive = Layer.succeed(GitHubService)({
		...
		Effect.tryPromise({
			try: async () => {
				...
				while (true) {
					...
					files.push(...data.map((f): ChangedFile => ({
						filename: f.filename,
	...
	commitFileChange: (owner: string, repo: string, path: string, content: string, sha: string, branch: string, message: string) =>
		Effect.tryPromise({
			try: async () => {
				const octokit = makeOctokit()
