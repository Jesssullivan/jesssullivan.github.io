...
export const handleCommand = (ctx: CommandContext) =>
	Effect.gen(function* () {
		...
		yield* github.reactToComment(ctx.owner, ctx.repo, ctx.commentId, "+1").pipe(
...
const handleSchedule = (ctx: CommandContext, dateStr: string) =>
	Effect.gen(function* () {
		...
		if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
			yield* github.postComment(ctx.owner, ctx.repo, ctx.prNumber,
		...
		if (marker.test(currentBody)) {
			newBody = currentBody.replace(marker, `DO NOT MERGE until ${dateStr} UTC`)
		} else {
			newBody = `${currentBody}\n\n---\n⚠️ **DO NOT MERGE until ${dateStr} UTC** — Scheduled for auto-merge.`
		}

		// Update PR body by posting a comment (we can't update PR body via issues API easily)
		yield* github.postComment(ctx.owner, ctx.repo, ctx.prNumber,
			`📅 Scheduled for **${dateStr} UTC**. The auto-merge workflow runs daily at 05:00 UTC.\n\nPR body updated with: \`DO NOT MERGE until ${dateStr} UTC\``)
...
const handlePublish = (ctx: CommandContext, fileArg: string, publish: boolean) =>
	Effect.gen(function* () {
		...
		const postFiles = files.filter(
			(f) => f.filename.startsWith("src/posts/") &&
		...
		const targets = fileArg
			? postFiles.filter((f) => f.filename.includes(fileArg))
			: postFiles
		...
		for (const file of targets) {
			...
			const newContent = content.replace(
				/^(published:\s*)(true|false)/m,
				`$1${publish}`
			)

			if (newContent !== content) {
				yield* github.commitFileChange(
					ctx.owner, ctx.repo, file.filename,
					newContent, "", pr.headRef,
		...
		if (updated.length > 0) {
			...
			if (publish) {
				yield* github.removeLabel(ctx.owner, ctx.repo, ctx.prNumber, "draft-post").pipe(
					Effect.catch(() => Effect.void)
				)
			} else {
...
const handleRetitle = (ctx: CommandContext, args: string) =>
	Effect.gen(function* () {
		...
		if (!titleMatch) {
			yield* github.postComment(ctx.owner, ctx.repo, ctx.prNumber,
				`⚠️ Usage: \`/retitle "New Title" [filename]\``)
		...
		const targets = fileArg
			? postFiles.filter((f) => f.filename.includes(fileArg))
			: postFiles.slice(0, 1) // Default to first post

		if (targets.length === 0) {
			yield* github.postComment(ctx.owner, ctx.repo, ctx.prNumber,
				`⚠️ No matching blog post files found.`)
			return { handled: true, response: "No files found" } satisfies CommandResult
		}
...
const handleSuggestLinks = (ctx: CommandContext) =>
	Effect.gen(function* () {
		...
		if (allSuggestions.length === 0) {
			...
			const body = `### 🔗 Hyperlink Suggestions\n\nFound **${allSuggestions.length}** unlinked keyword(s):\n\n` +
				allSuggestions.map((s) =>
					`- **${s.file}** line ${s.line}: \`${s.keyword}\` → \`[${s.keyword}](${s.url})\``
				).join("\n")
