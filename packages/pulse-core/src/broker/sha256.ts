...
// Synchronous SHA-256 keeps projection derivation usable from Node and browser
// bundles without pulling `node:crypto` into client-side hydration.
export const sha256Digest = (text: string): string => {
	...
	const hex = hash.map((word) => word.toString(16).padStart(8, '0')).join('');
	return `sha256:${hex}`;
};
