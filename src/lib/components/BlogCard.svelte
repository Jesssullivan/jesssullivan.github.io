<script lang="ts">
	...
	const dimFiles = import.meta.glob('/static/images/posts/dimensions.json', {
		eager: true,
		import: 'default',
		...
		<a href="/blog/{post.slug}" class="block">
			...
			{#if post.feature_image}
				...
				<div class="relative w-full h-48 overflow-hidden bg-surface-200-800">
					{#if post.featured}
						<div
							class="absolute top-3 left-3 z-10 flex items-center gap-1 text-xs font-semibold bg-primary-500 text-white px-2 py-1 rounded-full shadow-sm"
						>
			...
			<div class="p-6 {post.feature_image ? '' : 'pt-6'}">
				...
				<div class="flex items-center gap-3 text-sm text-surface-500 mb-2">
					...
					{#if post.reading_time}
						...
						<span class="text-xs" title="~{(post.reading_time ?? 1) * 230} words">{post.reading_time} min</span>
					{/if}
					{#if post.category}
						<span class="badge {CATEGORY_COLORS[post.category] ?? 'preset-outlined-surface-500'} text-xs capitalize"
							>{post.category}</span
						>
					{/if}
				</div>
			...
			<div class="flex flex-wrap gap-1.5 px-6 pb-5 -mt-2">
				{#each post.tags as tag (tag)}
					<a
						...
						class="badge preset-outlined-primary-500 text-xs hover:preset-filled-primary-500 transition-colors">{tag}</a
					>
				{/each}
			</div>
		{/if}
