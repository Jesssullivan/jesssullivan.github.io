...
The SSH-side workaround that got me moving was the local accessory restriction preference:
...
printf '%s\n' "$SUDO_PASS" | ssh -tt "$TARGET" \
  "sudo -S -p '' /bin/sh -c '
    /usr/bin/defaults write /Library/Preferences/com.apple.applicationaccess allowUSBRestrictedMode -bool true
...
When done, either wipe the Mac as planned or undo the local override:
